...
def forward_model(particles, _obs_op=obs_op):
    return particles @ _obs_op
...
# %% Initialization
init_particles = torch.randn((n_particles, 2)) / model.prior_precision + torch.tensor(
    [3.2, -5.0]
)
...
# %%
n_steps_hmc = n_steps - 100
pyro_model = pyro_tools.PyroModel(model, 2)
...
msip_quadgrad = MSIPQuadGradientInformed(
    post_log_dens_grad_val_batch, mc_quad_rule, gradient_decay
)
