...
DIM = 2
post_log_dens = funnel.logpdf_factory(DIM)
...
# Same spirit as your older Funnel script: initialize from N(0, I).
...
def make_ksd_metric(kernel_name: str, sigma: float):
    """Build KSD robustly, using vectorized scores when supported."""
    kwargs = {
        "kernel_length_scale": sigma,
    }
    if kernel_name == "IMQ":
        kwargs["kernel_elem"] = imq_kernel
...
def mc_quad_rule(batch_size: int, N_quad: int = N_QUAD, dim: int = DIM):
...
try:
    ...
    grid_res = 200
    x_range = torch.linspace(-8, 8, grid_res)
    y_range = torch.linspace(-8, 8, grid_res)
    ...
    dens_grid = torch.exp(
        log_dens_grid - log_dens_grid.max()
    ).detach().cpu().numpy()
    ...
    fig.suptitle(f"Final particles — Funnel 2D (M={M}, T={T}, run {R}/{R})", fontsize=13, y=1.02)
    plt.tight_layout()
