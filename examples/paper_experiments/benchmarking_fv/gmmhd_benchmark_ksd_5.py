...
def mc_quad_rule(batch_size: int, N_quad: int = 1, dim: int = 2):
    """
...
def run_one_algorithm(
    ...
    if algo_name == "MSIP-GI-1":
        msip_qg = MSIPQuadGradientInformed(
            ...
            partial(mc_quad_rule, N_quad=1, dim=d),
            gradient_decay,
        )
        trajectories, traj_wts = msip(
            msip_qg,
    ...
    if algo_name == "MSIP-GI-10":
        msip_qg = MSIPQuadGradientInformed(
            post_log_dens_grad_val_batch,
            partial(mc_quad_rule, N_quad=10, dim=d),
            gradient_decay,
        )
        trajectories, traj_wts = msip(
            msip_qg,
