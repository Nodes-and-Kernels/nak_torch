...
# Same spirit as your original Himmelblau script: initialize from N(0, I).
init_mean = torch.tensor([0.0, 0.0], dtype=torch.get_default_dtype())
init_std = 1.0
base_seed = 314159
...
try:
    ...
    for ax_idx, algo_name in enumerate(algo_names):
        ...
        if wts is not None:
            ...
            sizes = np.clip(wts_np * M * 200, 10, 300)
            ax.scatter(xy[:, 0], xy[:, 1], s=sizes, alpha=0.8, zorder=3, linewidths=0.4)
        else:
            ax.scatter(xy[:, 0], xy[:, 1], s=40, alpha=0.8, zorder=3, linewidths=0.4)
    ...
    fig.suptitle(f"Final particles — Himmelblau (M={M}, T={T}, run {R}/{R})", fontsize=13, y=1.02)
    plt.tight_layout()
