...
#    "MSIP-GS-QG":    traj_wts_msip_gs_qg,
    "MSIP-GMM":      traj_wts_msip_gmm,
#    "MSIP-GS-GMM":   traj_wts_msip_gs_gmm,
}

#metrics        = {name: {"mmd": [], "ksd_rbf": [], "ksd_imq": []} for name in algo_trajs}

metrics = {
    name: {"mmd": [], "ksd_rbf": [], "ksd_imq": [], "cross_entropy": []}
...
ax.set_title("Cross entropy vs Iteration – gradient-informed algorithms")
ax.legend(fontsize=8, ncol=2)
plt.tight_layout()
plt.savefig("gi_cross_entropy_all_algorithms_sigma_" + str(kernel_length_scale) + ".pdf")
plt.close()
print("Saved gi_cross_entropy_all_algorithms_sigma_" + str(kernel_length_scale) + ".pdf")
