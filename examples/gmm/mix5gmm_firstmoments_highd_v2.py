...
Target
──────
  Five-component Gaussian mixture in R^d, with d >= 5.
  The component means are alpha * e_1, ..., alpha * e_5.
  The parameter alpha is MODE_SEPARATION_ALPHA below.
...
───────
  ...
  |Ehat[f] - E_true[f]| for f in:
    ...
    selected cross moments among the first five coordinates
...
def make_axis_gmm(d: int, alpha: float = MODE_SEPARATION_ALPHA):
    ...
    Therefore this construction requires d >= 5. The remaining coordinates
    6,...,d have zero mean for all components. The default covariance is
    TARGET_COV_SCALE * I_d for every component.
    """
    if d < N_COMPONENTS:
...
def setup_gmm(d: int):
    ...
    # Selected cross moments among the five active coordinates.
    # For this axis-GMM with isotropic covariances, these are nonzero only
    # through products of component means; since each mode lives on one axis,
    # E[x_i x_j] = 0 for i != j. Still useful as diagnostics.
    candidate_pairs = [(0, 1), (0, 2), (0, 4), (3, 4)]
...
def main():
    for d in D_VALUES:
        print(f"\n\n{'#' * 70}\nDIMENSION d = {d}\n{'#' * 70}")
        setup_gmm(d)
        print(f"Using alpha = {MODE_SEPARATION_ALPHA}, init alpha = {INIT_ALPHA}")
